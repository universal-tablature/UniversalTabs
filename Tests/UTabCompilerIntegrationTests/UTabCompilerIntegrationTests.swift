...
@Test func looseTwinkleFixtureProducesDeterministicJSONAndMIDI() throws {
    ...
    let lilyPond = firstDirectory.appendingPathComponent("twinkle.ly")
    ...
    try UTabCompilerCommand.execute(["--emit", "lilypond", fixture.path, "-o", lilyPond.path])
    ...
    let lilyPondSource = try String(contentsOf: lilyPond, encoding: .utf8)
    #expect(lilyPondSource.contains("\\score"))
    #expect(lilyPondSource.contains("\\new Staff"))
    #expect(lilyPondSource.contains("\\clef \"treble_8\""))
    #expect(lilyPondSource.contains("\\clef \"bass_8\""))
    #expect(lilyPondSource.contains("<e, g c'>4*4/1"))
    #expect(lilyPondSource.contains("\\new Lyrics \\lyricsto"))
    #expect(lilyPondSource.contains("\"Twin\" -- \"kle\""))
